...
if uploaded_file:
    ...
    # Convert types
    date_cols = ['Inquiry Created Date', 'Site Visit Date Time', 'Latest Quotation Date', 'Task Created Date', 'Completed Date', 'Claimed Date']
    for col in date_cols:
        if col in df.columns:
            df[col] = pd.to_datetime(df[col], errors='coerce')

    float_cols = ['Advance Amount', 'Additional Discount', 'Latest Quoted Inverter Capacity (kW)', 'Latest Final Investment', 'DC Capacity']
    ...
    # Assign Sales BDO (logic already present)
    ...
    # Final fix: Replace Sales BDO with RSM Lead or BDM Lead if matched
    def correct_sales_bdo(row):
        if row.get('Sales BDO') == row.get('RSM'):
            return 'RSM Lead'
        elif row.get('Sales BDO') == row.get('BDM'):
            return 'BDM Lead'
